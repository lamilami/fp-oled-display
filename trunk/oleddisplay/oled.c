...
void oled_init(void) {
	...
	oled_send(0x00, 0); /*to horizontal mode (auto linebreak)*/

	oled_send(0x21, 0); //set column address
	oled_send(0x01, 0); //set column start address 1
	oled_send(0x7E, 0); //set column end address 126

	oled_send(0x22, 0); //set page address
	oled_send(0x00, 0); //set start page 0
	oled_send(0x00, 7); //set last page 7

	oled_send(0x81, 0); /*contrast control*/
	oled_send(0x80, 0); /*128*/

	oled_send(0xA0, 0); /*set segment remap*/
...
void oled_draw_charcol(char c, uint8_t x, uint8_t y) {
	if (x > 127 || y > 63) return;
...
void oled_putc(char c, uint8_t line, uint8_t column) {
	if ((line >= 22) || (column >= 9)) return; // Clip right and bottom

	uint8_t pixelline;
	for (uint8_t i = 0; i < 6; i++) {
		if (i == 5) pixelline = 0x00;
		else pixelline = pgm_read_byte(oledchars + (c * 5) + i);
		oled_draw_charcol(pixelline, (column - 1) * 6 + i, (line - 1) * 8);
	}
}
